...
def evaluate_nn(model, loader):
    ...
    with torch.no_grad():
        for vid, aud, lbls in loader:
            ...
            all_probs.extend(probs[:, 1].cpu().numpy())  # Positive (Deceptive) probability
    return accuracy_score(all_labels, all_preds), np.array(all_probs), np.array(all_labels)

if __name__ == "__main__":
    print("\n==========================================================")
    print(" HETEROGENEOUS SOFT-VOTING ENSEMBLE [NN + RF + SVM] ")
    print("==========================================================")
    ...
    # 1. Train Scikit-Learn Classifiers (RF + SVM)
    print("\n[1/3] Training Algorithmic Support Vectors & Decision Trees...")
    ...
    print(f"      Support Vector Machine Accuracy:  {accuracy_score(y_test, svm_preds):.4f}")
    print(f"      Random Forest Tree Accuracy:      {accuracy_score(y_test, rf_preds):.4f}")

    # 2. Train Deep PyTorch Tensor Variant
    ...
    for epoch in tqdm(range(1, 151), desc="Deep Training"):
        ...
        _, probs_val, lbls_val = evaluate_nn(model, val_loader)
        v_preds = (probs_val >= 0.5).astype(int)
        v_f1 = f1_score(lbls_val, v_preds, average='macro', zero_division=0)
    ...
    nn_acc, nn_probs, y_test_true = evaluate_nn(model, test_loader)
    print(f"      Neural Network Dual-Stream Accuracy: {nn_acc:.4f}")
    
    # 3. Soft-Voting Ensembling Aggregation
    print("\n[3/3] Synthesizing Matrix Aggregation Over Probabilities...")
    # Soft Voting = Mean Probabilistic Distributions directly canceling network variance
    ensemble_probs = (rf_probs + svm_probs + nn_probs) / 3.0
    ensemble_preds = (ensemble_probs >= 0.5).astype(int)
    
    ensemble_acc = accuracy_score(y_test_true, ensemble_preds)
    ensemble_f1 = f1_score(y_test_true, ensemble_preds, average='macro', zero_division=0)
    ensemble_auc = roc_auc_score(y_test_true, ensemble_probs)
    
    print("\n==========================================================")
    print(f" [NN + RF + SVM] ENSEMBLED TARGET ACCURACY: {ensemble_acc:.4f}")
    print(f"             Aggregated Macro F1 Algorithm: {ensemble_f1:.4f}")
    print(f"             Overall Test ROC Area Under Curve: {ensemble_auc:.4f}")
    print("==========================================================")
